    ...
    class UnmaskedView
    {
                ...
                foreach (var controlSelector in unmaskedControls[viewRects.Key])
                {
                    ...
                    switch (controlSelector.SelectorMode)
                    {
                        ...
                        case GuiControlSelector.Mode.Rect:
                            regionRects.Add( controlSelector.Rect );
                            break;
        ...
        Type ResolvedEditorWindowType
        {
            get
            {
                ...
                if ( m_EditorWindowType.m_TypeName.Contains( "LunaExportWindow" ) ) {
                    var windowByName = (EditorWindow)Resources.FindObjectsOfTypeAll(typeof(EditorWindow)).FirstOrDefault(u =>
                        u.GetType().Name.Contains( "LunaExportWindow" ));
                    if (windowByName)
                    {
                        return windowByName.GetType();
                    }
                }
